...
def pyvmi_rpc_send(VMname,funName,*funArg):
    ...
    pyvmi_rpc_list[0].send(VMname+" "+funName+" "+funArgType+" "+funArgString)
...
def list_processes(VMName):
    tasksOffset = int(pyvmi_rpc_send(VMName, "get_offset","linux_tasks"))
    nameOffset = int(pyvmi_rpc_send(VMName, "get_offset","linux_name"))
    pidOffset = int(pyvmi_rpc_send(VMName, "get_offset","linux_pid"))
    init_task_va = int(pyvmi_rpc_send(VMName, "translate_ksym2v","init_task"))
    processes = int(pyvmi_rpc_send(VMName, "read_addr_va",init_task_va+tasksOffset,0))
    print init_task_va+tasksOffset,processes
    current_process = processes
    while True:
        pid = int(pyvmi_rpc_send(VMName,"read_32_va",current_process+pidOffset-tasksOffset,0))
        procname = pyvmi_rpc_send(VMName,"read_str_va",current_process+nameOffset-tasksOffset,0)
        print pid, procname
        current_process = int(pyvmi_rpc_send(VMName,"read_addr_va",current_process,0))
        if current_process == processes:
            break
    return
#dom0 to be watched
dom0List=[["127.0.0.1",8005]]
#vm to be watched
vmList=[["VM2-ovs"]]
