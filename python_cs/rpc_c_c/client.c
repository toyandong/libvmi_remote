...
int main()
{
    ...
    rvmi_init(prvmi,VMI_AUTO | VMI_INIT_COMPLETE,"VM2-ovs");
    rvmi_get_offset(prvmi, "linux_tasks");
    rvmi_translate_ksym2v(prvmi, "init_task");
    //rvmi_read_addr_va(prvmi,);
    uint32_t tmp1;
    addr_t tmp2;
    rvmi_read_32_va(prvmi,3402072560,0,&tmp1);
    printf("tmp %d\n",tmp1);
    printf("name %s\n", rvmi_read_str_va(prvmi,3402072856,0));
    rvmi_destroy(prvmi);
