...
static int insert_vm_file(rvmi_instance_t *prvmi, addr_t vaddr, int pid, struct my_task_struct_offset_t *my_ts_off,int dom_id, char *dom_name)
{
    ...
    rvmi_read_addr_va(prvmi, fdt_ptr, 0, &max_fds );
    printf("max_fds : %d\n", max_fds);
    /*遍历打开的所有文件*/
    for(i=0; i<max_fds;i++)
    {
...
*/
   ...
   /*if (rvmi_pause_vm(vmi) != rvmi_SUCCESS)
    {
	...
	*/
    ...
    dom_name = name; // rvmi_get_name(vmi);
    dom_id = 1;
    //dom_id = rvmi_get_vmid(vmi);
    ...
    while (1){
        ...
        if (pid >= 0){
            printf("============================================================================\n");
            printf("[%5d] %s %d\n", pid, procname, state);
            insert_vm_process(prvmi, next_process-tasks_offset, pid, procname,&my_ts_off, dom_id, dom_name);
            insert_vm_file(prvmi, next_process-tasks_offset, pid, &my_ts_off, dom_id, dom_name);
            insert_process_create(prvmi, next_process-tasks_offset, pid, &my_ts_off, dom_id, dom_name);
